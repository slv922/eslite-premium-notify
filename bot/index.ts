...
// Extract booking code from a raw code or URL
function parseBookingCode(input: string): string | null {
  const text = input.trim()

  // URL: take last path segment that looks like a code
  if (text.startsWith('http')) {
    const match = text.match(/\/([A-Z0-9]{4,12})(?:[/?#]|$)/i)
    return match ? match[1].toUpperCase() : null
  }

  // Raw code: 4–12 alphanumeric chars
...
bot.command('track', async ctx => {
  ...
  if (!input) {
    return ctx.reply('請輸入訂位代碼，例如：/track EEHWAS')
  }
  const code = parseBookingCode(input)
  if (!code) {
    return ctx.reply('無法識別訂位代碼，請確認格式是否正確。')
  }
...
bot.command('status', async ctx => {
  ...
  if (!session) {
    return ctx.reply('目前沒有追蹤中的訂位，請先傳送訂位代碼。')
  }
